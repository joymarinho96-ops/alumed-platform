...
// --- VISOR DE MUESTRAS ALUMED LOGIC ---
function abrirVisorAlumed(btnEl) {
  document.querySelectorAll('.tab-content').forEach(el => el.classList.remove('active'));
  document.querySelectorAll('.nav-btn').forEach(el => el.classList.remove('active'));
  
  const sec = document.getElementById('tab-visor-alumed');
  if (sec) sec.classList.add('active');
  if (btnEl) btnEl.classList.add('active');
  
  renderAlumedAtlasGrid();
}

function renderAlumedAtlasGrid() {
  const container = document.getElementById('alumedAtlasGrid');
  if (!container) return;
  
  const dataset = window.ALUMED_PROPRIO_ATLAS_DATA || [];
  if (dataset.length === 0) {
    container.innerHTML = '<p style="color:#94a3b8; grid-column: 1/-1;">No hay imágenes cargadas.</p>';
    return;
  }
  
  container.innerHTML = dataset.map(item => `
    <div class="atlas-card" onclick="abrirAtlasModalALUMED('${item.id}')">
      <div class="atlas-card-img-wrap">
        <img src="${item.urlImagem}" alt="${item.titulo}" loading="lazy">
        <span class="atlas-card-badge">ALUMED</span>
      </div>
      <div class="atlas-card-body">
        <h3 class="atlas-card-title">${item.titulo}</h3>
        <div class="atlas-card-footer">
          <span style="font-size: 0.78rem; color: var(--cyan-neon); font-weight: 700;">
            <i class="fa-solid fa-eye"></i> Ver Muestra
          </span>
          <i class="fa-solid fa-chevron-right" style="font-size: 0.8rem; color: var(--text-muted);"></i>
        </div>
      </div>
...
function abrirAtlasModalALUMED(id) {
  const dataset = window.ALUMED_PROPRIO_ATLAS_DATA || [];
  const item = dataset.find(s => s.id === id);
  if (!item) return;
  ...
  document.getElementById('atlasModalCategory').textContent = "ATLAS ALUMED";
  document.getElementById('atlasModalSlideTitle').textContent = item.titulo;
  
  const subHtml = `
    <div style="margin-top: 4px; font-size: 0.85rem; color: #cbd5e1;">
      <div><strong>Fuente:</strong> Archivos ALUMED</div>
    </div>
  `;
  document.getElementById('atlasModalSpecimen').innerHTML = subHtml;
  document.getElementById('atlasModalImg').src = item.urlImagem || '';
  
  // Hide live button because it's a static image
  const liveBtn = document.getElementById('atlasTabLiveEmbed');
...
// --- FIN VISOR DE MUESTRAS ALUMED LOGIC ---
